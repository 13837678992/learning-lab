 ...
 * qiankun 应用名须与 @fmac/constants 的 MICRO_APPS.USER 一致。
 */
const QIANKUN_NAME = 'app-user';

export default defineConfig(({ mode }) => {
  const env = loadEnv(mode, process.cwd(), 'VITE_');
  return {
    // 部署基址：standalone/独立部署用此 base；qiankun 运行期由插件动态注入 public-path。
    base: env.VITE_BASE || '/',
    plugins: [vue(), qiankun(QIANKUN_NAME, { useDevMode: true })],
    server: {
      port: 7101,
      cors: true,
      origin: env.VITE_DEV_ORIGIN || 'http://localhost:7101',
    },
    preview: { port: 7101, cors: true },
    build: {
      target: 'es2015',
