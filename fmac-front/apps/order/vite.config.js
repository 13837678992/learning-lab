 ...
 * qiankun 应用名须与 @fmac/constants 的 MICRO_APPS.ORDER 一致。
 */
const QIANKUN_NAME = 'app-order';
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      port: 7102,
      cors: true,
      origin: env.VITE_DEV_ORIGIN || 'http://localhost:7102',
    },
    preview: { port: 7102, cors: true },
    build: {
      target: 'es2015',
