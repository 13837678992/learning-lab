 ...
 * - entry：来自 @fmac/env（按 mode 解析），并允许 import.meta.env.VITE_SUB_* 覆盖以适配部署。
 */
import { MICRO_APPS, SUBAPPS, SUBAPP_CONTAINER } from '@fmac/constants';
import { resolveEnv } from '@fmac/env';

const ENTRIES = resolveEnv(import.meta.env.MODE).SUBAPPS;

/** 部署期可经 Vite 环境变量覆盖各子应用 entry。 */
const ENTRY_OVERRIDES = {
  [MICRO_APPS.USER]: import.meta.env.VITE_SUB_USER,
  [MICRO_APPS.ORDER]: import.meta.env.VITE_SUB_ORDER,
  [MICRO_APPS.REPORT]: import.meta.env.VITE_SUB_REPORT,
  [MICRO_APPS.FINANCE]: import.meta.env.VITE_SUB_FINANCE,
};
