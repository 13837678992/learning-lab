...
class plt_one_addpt_onclick:
    ...
    def resize_sq(self, bcid):
        ...
        elif hasattr(bcid, '_rectangles'):
            rect = bcid._rectangles[0]
            h = rect.get_height()
            rect.set_height(3 * h)
        ...
        elif hasattr(bcid, '_checks'):
            # 新版是 PathCollection，取 size，注意：size 是面积，单位不一样
            sizes = bcid._checks.get_sizes()
            print(sizes)
            if len(sizes) > 0:
                h = sizes[0]
                bcid._checks.set_sizes([3 * h for _ in sizes])
                print(bcid._checks.get_sizes())

                # 取 offset 的 y 值上下限
                offsets = bcid._checks.get_offsets()
                y_vals = offsets[:, 1]
                ymax = max(y_vals)
                ymin = min(y_vals)
            else:
                print("CheckButtons 的 _checks 没有 size")
                h, ymax, ymin = None, None, None

        else:
            print("CheckButtons 没有可用的矩形属性")
            h, ymax, ymin = None, None, None
        ...
        if hasattr(bcid, 'lines'):
            ...
            offsets = bcid._checks.get_offsets()
            offsets[0, 0] = ymax  # 第一个点的 y 改成 ymax
            offsets[0, 1] = ymin  # 第二个点的 y 改成 ymin
            bcid._checks.set_offsets(offsets)
