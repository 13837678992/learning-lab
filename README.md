...
| `qrcode-transfer-web` | 同款工具的轻量单文件网页版。构建产物仅约 40KB 的单个 HTML，双击浏览器即用，功能与桌面版一致，适合内网快速分发。 |
| `qrcode-multi-scanner` | 上面两个工具的 Android 接收端（Kotlin + Compose + CameraX + ML Kit）。按顺序逐张扫描分片二维码，每张内容独立保存，最后拼回完整文本，可复制或导出 TXT。完全离线、无网络权限、无账号、无后端。 |
